...
class DataManager:
    ...
    def _zoom_to_layer(self, layer: QgsVectorLayer) -> None:
        """
        レイヤの範囲に地図表示をズームする（v1.9.1改訂 - CRS変換対応）
        ...
        Note:
            ...
            OSMレイヤー（EPSG:3857）と電柱設備レイヤー（EPSG:4326）の
            CRS不一致に対応するため、座標変換を実施。
        """
        if not layer or not layer.isValid():
            return
        ...
        if layer_crs != canvas_crs:
            ...
            Logger.info(
                f"CRS変換実行: {layer_crs.authid()} → {canvas_crs.authid()}"
            )
